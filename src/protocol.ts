...
export type ConnectStreamLine =
  ...
  | { type: 'connected'; alias: string; latencyMs: number; home: string; workspace?: string }
  | { type: 'failed'; alias: string; error: string }
...
export const SSH_API_BASE = '/api/remote-ssh' as const

export const SSH_API = {
  hosts: SSH_API_BASE + '/hosts',
  sshAliases: SSH_API_BASE + '/ssh-aliases',
  importSshConfig: SSH_API_BASE + '/hosts/import-ssh-config',
  recents: SSH_API_BASE + '/recents',
  status: SSH_API_BASE + '/status',
  session: SSH_API_BASE + '/session',
  workspace: SSH_API_BASE + '/workspace',
  connect: SSH_API_BASE + '/connect',
  disconnect: SSH_API_BASE + '/disconnect',
  exec: SSH_API_BASE + '/exec',
  ls: SSH_API_BASE + '/ls',
  read: SSH_API_BASE + '/read',
  write: SSH_API_BASE + '/write',
  mkdir: SSH_API_BASE + '/mkdir',
  rename: SSH_API_BASE + '/rename',
  remove: SSH_API_BASE + '/remove',
...
export function withCwd(command: string, cwd?: string): string {
  ...
  const safe = cwd.trim().replaceAll("'", `'"'"'`)
  return `cd -- '${safe}' && ( ${command} )`
}
